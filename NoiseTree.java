...
public class NoiseTree {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (MaxPowerOfTwo < 0 || alpha <0 || alpha >= 1 || (version != 1 && version != 2) || validationInPercent < 0 || validationInPercent > 100) {
                throw new Exception();
            }
        } catch (Exception e) {
        ...
        String trainingFileName = args[4];
    ...
    private static void ApplyNoise(Example example, double alpha){
        for (int i = 0; i < example.pixels.length; i++) {
            double noise = Math.random();
            if (noise < alpha) {
                example.pixels[i] = (short) (256 - example.pixels[i]);
            }
        }
    }
